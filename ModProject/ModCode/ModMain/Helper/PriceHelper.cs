...
public static class PriceHelper
{
    public static readonly IDictionary<ModItemTypeEnum, float> ITEM_TYPE_RATIO = new Dictionary<ModItemTypeEnum, float>
    {
        ...
        [ModItemTypeEnum.Artifact] = 1.24f,
        [ModItemTypeEnum.TalismanRecipe] = 1.18f,
        [ModItemTypeEnum.PillRecipe] = 1.18f,
        [ModItemTypeEnum.Mount] = 1.20f,
        [ModItemTypeEnum.Ring] = 1.16f,
        [ModItemTypeEnum.Furnace] = 1.14f,
        [ModItemTypeEnum.BottleneckItem] = 1.10f,
        [ModItemTypeEnum.PowerUpItem] = 1.08f,
        [ModItemTypeEnum.Potion] = 1.05f,

        [ModItemTypeEnum.Herb] = 1.10f,
        [ModItemTypeEnum.Mine] = 1.12f,
    ...
    public static int UpPrice(int cur, int grade, int level, float customRatio = 1.00f)
    {
        grade = Math.Max(Math.Min(grade - 1, 10), 0);
        level = Math.Max(Math.Min(level - 1, 6), 0);
        var upPrice = cur.Parse<float>();
        upPrice *= GRADE_RATIO[grade];
        upPrice *= LEVEL_RATIO[level];
        upPrice *= customRatio;
        return upPrice.Parse<int>();
    }
}